...
void CDE_init(char** argv, int optind) {
  ...
  if (CDE_exec_mode) {
    ...
    if (stat("ptu.options", &cde_options_stat)) {
      FILE* f = fopen("ptu.options", "w");
  ...
  if (!CDE_exec_mode) {
    ...
    struct stat tmp;
    FILE* log_f;
    char* log_filename = format("%s/ptu.log", CDE_PACKAGE_DIR);
    ...
    char* fullenviron_fn = format("%s/ptu.full-environment.%s", CDE_PACKAGE_DIR, CDE_ROOT_NAME);
...
static void CDE_init_options() {
  ...
  if (CDE_exec_mode) {
    ...
    char* options_file = format("%s/../ptu.options", cde_pseudo_root_dir);
    f = fopen(options_file, "r");
    if (!f) {
      fprintf(stderr, "Fatal error: missing ptu.options file\n");
    ...
    f = fopen("ptu.options", "r");

    // if found, copy it into the package
    if (f) {
      char* fn = format("%s/ptu.options", CDE_PACKAGE_DIR);
      copy_file((char*)"ptu.options", fn, 0666);
      free(fn);
    }
    else {
      fprintf(stderr, "Fatal error: missing ptu.options file\n");
  ...
  while ((read = getline(&line, &len, f)) != -1) {
    ...
    if (is_first_line) {
      if (strncmp(line, CDE_OPTIONS_VERSION_NUM, strlen(CDE_OPTIONS_VERSION_NUM)) != 0) {
        fprintf(stderr, "Error: ptu.options file incompatible with this version of cde ('%s')\n",
    ...
    for (p = strtok(line, "="); p; p = strtok(NULL, "=")) {
      if (is_first_token) {
        if (strcmp(p, "ignore_exact") == 0) {
          ...
          if (!in_braces) {
            fprintf(stderr, "Fatal error in ptu.options: 'process_ignore_prefix' must be enclosed in { } after an 'ignore_process' directive\n");
            exit(1);
          }
          set_id = 100;
        }
        else {
          fprintf(stderr, "Fatal error in ptu.options: unrecognized token '%s'\n", p);
          exit(1);
        }

        if (in_braces && set_id != 100) {
          fprintf(stderr, "Fatal error in ptu.options: Only 'process_ignore_prefix' is allowed within { } after an 'ignore_process' directive\n");
          exit(1);
        }
        ...
        switch (set_id) {
          ...
          case 100:
            ...
            if (cur->process_ignore_prefix_paths_ind >= 20) {
              fprintf(stderr, "Fatal error in ptu.options: more than 20 'process_ignore_prefix' entries\n");
              exit(1);
            }
            break;
          default:
